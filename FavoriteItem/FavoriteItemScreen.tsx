...
import styles from "./FavoriteItemScreen.styles";
import navigationOptions from "./FavoriteItemScreen.navigationOptions";

const FavoriteItemScreen = ({ language = "" }: LocalizationContextProps) => {
  ...
  let emission = useSelector((state) => emissions.selectors.getFoodById(state, emissionId));
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {foodType == FoodsType.custom || foodType == FoodsType.productScanned ? (
        <Text.Primary darkGray style={styles.item}>
          {ui.getTranslationFoodsType(foodType)}
        </Text.Primary>
      ) : (
        <Text.Primary darkGray style={styles.item}>
          {ui.getTranslationFoodsType(foodType)}
      ...
      <Text.Primary darkGray style={styles.item}>
        <FormattedNumber
          ...
          value={co2Emission > 1 ? co2Emission * 10: co2Emission * 1000}
        />{" "}
        {co2Emission > 1 ? " calories" : " calories"}
      </Text.Primary>
...
FavoriteItemScreen.navigationOptions = navigationOptions;

export default withLocalization(FavoriteItemScreen);
