...
import { FoodsType, Emission, RecurringEmission, PeriodicityType, WeekDays } from "interfaces";
...
const getC02ValueFromEmission = (emission: Emission | RecurringEmission): number => {
  ...
  if (emission.foodType === FoodsType.electricity) {
    return emission.value * electricity[emission.foodModelType];
  }

  if (emission.foodType === FoodsType.streaming) {
    return getInternetUsageCarbonImpact(
      emission.value,
      streaming[emission.foodModelType] * emission.value,
  ...
  return emission.value * model[emission.foodModelType];
};
