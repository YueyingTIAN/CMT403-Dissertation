...
import { Emission, FoodsType } from "interfaces";
...
const emissionFood: Emission = {
  ...
  foodModelType: FoodsType.beans,
  foodType: FoodsType.food,
  isMitigated: false,
  value: 10,
};
...
const emissionTransport: Emission = {
  ...emissionFood,
  foodModelType: TransportType.boat,
  foodType: FoodsType.transport,
};

const emissionStreaming: Emission = {
  ...emissionFood,
  foodModelType: StreamingType.HDVideo,
  foodType: FoodsType.streaming,
  location: ElectricityType.argentina,
};

const emissionElectricity: Emission = {
  ...emissionFood,
  foodModelType: ElectricityType.france,
  foodType: FoodsType.electricity,
};

const emissionPurchase: Emission = {
  ...emissionFood,
  foodModelType: PurchaseType.computer,
  foodType: FoodsType.purchase,
};

const emissionFashion: Emission = {
  ...emissionFood,
  foodModelType: FashionType.coat,
  foodType: FoodsType.fashion,
};

const emissionMeal: Emission = {
  ...emissionFood,
  foodModelType: MealType.highMeat,
  foodType: FoodsType.meal,
};

const emissionCustom: Emission = {
  ...emissionFood,
  foodModelType: "custom",
  foodType: FoodsType.custom,
};

const emissionProductScanned: Emission = {
  ...emissionFood,
  foodModelType: "productScanned",
  foodType: FoodsType.custom,
};
...
describe("getC02ValueFromEmission should return the correct co2 emitted value for", () => {
  ...
  it("streaming emission", () => {
    expect(calculation.getC02ValueFromEmission(emissionStreaming)).toEqual(
      getInternetUsageCarbonImpact(
        ...
        streaming[emissionStreaming.foodModelType] * emissionStreaming.value,
  ...
  it("electricity emission", () => {
    expect(calculation.getC02ValueFromEmission(emissionElectricity)).toEqual(
      emissionElectricity.value * electricity[emissionElectricity.foodModelType]
    );
  });
  it("custom emission", () => {
